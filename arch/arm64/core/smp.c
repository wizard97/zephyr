...
/*
 * How long to wait for a CPU to take the freeze SGI and snapshot its
 * registers before giving up on it and dumping without it. Bounded by the
 * free-running counter (k_cycle_get_32(), a bare CNTVCT_EL0 read on arm64 --
 * no scheduler, no interrupt, no lock, same as arch_busy_wait()), so it is a
 * calibrated wall-clock value that behaves identically on fast and slow
 * cores. The tick/scheduler can't be trusted while another CPU is panicking,
 * but the counter keeps advancing regardless, so it is safe here. A CPU that
 * has not responded within this window is almost certainly mid-critical-
 * section with IRQs masked, in which case no longer wait would help.
 */
#define COREDUMP_FREEZE_TIMEOUT_MS	100
...
void arch_coredump_freeze_other_cpus(void)
{
	...
	for (unsigned int i = 0; i < num_cpus; i++) {
		uint32_t start = k_cycle_get_32();
		uint32_t max_cycles = (uint32_t)k_ms_to_cyc_ceil32(COREDUMP_FREEZE_TIMEOUT_MS);

		if (i == self) {
			continue;
		}

		while (atomic_get(&freeze_state[i]) == FREEZE_REQUESTED) {
			if ((uint32_t)(k_cycle_get_32() - start) >= max_cycles) {
				break;
			}
		}
